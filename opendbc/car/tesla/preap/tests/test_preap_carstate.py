...
class TestPreAPDIStateDecode(unittest.TestCase):
  ...
  speed from bits 32-40 made no-pedal ACC see roughly half the true set speed,
  so it computed a large positive offset and demanded UP_2ND every 500 ms —
  with presses live that would ratchet the real set speed to the TX ceiling.
  ...
  set-speed change (stalk step, or a future no-pedal ACC spoof) causes a few
