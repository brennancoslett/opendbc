...
STALK_UP_2ND = 4    # RES_ACCEL_2ND — no-pedal ACC exclusive
STALK_DN_1ST = 32   # DECEL_SET — also used by the base renorm FSM
STALK_DN_2ND = 8    # DECEL_2ND — no-pedal ACC exclusive
...
class TestTeslaPreAPStalkButtonGating(TeslaPreAPTestMixin, unittest.TestCase):
  """Panda-side backstop for no-pedal ACC stalk speed buttons.

  UP_2ND/DN_2ND (RES_ACCEL_2ND/DECEL_2ND) are only ever sent by the no-pedal ACC
  ...
  speed-adjust values are additionally rate-limited while no-pedal ACC is
  active, as a floor beneath no_pedal_acc.py's own 500ms spacing gate.
  """
  __test__ = True
  ...
  def test_2nd_detent_buttons_allowed_with_flag(self):
    for lever in (STALK_UP_2ND, STALK_DN_2ND):
      ...
      self.assertTrue(self._tx(self._stalk_button_msg(lever)),
                      f"lever {lever} must be allowed with PREAP_FLAG_NO_PEDAL_ACC")
  ...
  def test_1st_detent_buttons_unaffected_by_flag(self):
    ...
    # keep working regardless of the no-pedal ACC flag.
    self._setup_safety_hooks(no_pedal_acc=False)
    self.safety.set_controls_allowed(True)
    for lever in (STALK_UP_1ST, STALK_DN_1ST):
      self.assertTrue(self._tx(self._stalk_button_msg(lever)),
                      f"lever {lever} must not require PREAP_FLAG_NO_PEDAL_ACC")
  ...
  def test_speed_button_rate_limited_with_no_pedal_acc(self):
    self.safety.set_controls_allowed(True)
    # Past the rate-limit floor from a fresh (timer=0) safety init.
