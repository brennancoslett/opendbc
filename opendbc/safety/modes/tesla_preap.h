...
//   - Stalk speed-button TX (0x45) value-validated, no-pedal ACC-exclusive
//     buttons gated by PREAP_FLAG_NO_PEDAL_ACC, and rate-limited while active
//
...
#define PREAP_FLAG_NO_PEDAL_ACC         8U
...
static bool preap_no_pedal_acc = false;
...
// No-pedal ACC speed-button TX backstop. RES_ACCEL_2ND/DECEL_2ND (UP_2ND/DN_2ND)
// are only ever sent by the no-pedal ACC speed modulator (base engage/cancel/
...
static bool tesla_preap_tx_hook(const CANPacket_t *msg) {
  ...
  // STW_ACTN_RQ (0x45): stalk-button spoof. See PREAP_FLAG_NO_PEDAL_ACC comment
  ...
  if (msg->addr == 0x45U) {
    ...
    if (!valid_lever) {
      ...
      // RES_ACCEL_2ND / DECEL_2ND: no-pedal ACC-exclusive
      if (!preap_no_pedal_acc) {
        violation = true;
      }
    }
...
static safety_config tesla_preap_init(uint16_t param) {
  ...
  preap_no_pedal_acc = GET_FLAG(param, PREAP_FLAG_NO_PEDAL_ACC);
